...
function MusicItem({ item, index }: { item: Music; index: number }) {
  return (
    <div
    ...
    >
      ...
      <span className={'text-sm w-1/4 text-gray-400 cursor-pointer hover:underline'}>
        <a href={`${ALBUM_HOST}${item.album_name}`} target="_blank" rel="noreferrer">
          《{item.album_name}》
        </a>
      </span>

      <span className={'text-sm w-1/4 text-gray-400 text-center cursor-pointer  hover:underline'}>
